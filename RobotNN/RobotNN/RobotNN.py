...
V_goal = tf.constant([[0.06,0.,0.]], dtype = tf.float64)

#Variables
global_step = tf.Variable(0,trainable = False, name = 'global_step')
W1=tf.Variable(tf.random_uniform([33,20], -0.01, 0.01,dtype=tf.float64), dtype=tf.float64)
W2=tf.Variable(tf.random_uniform([20,12], -0.01, 0.01,dtype=tf.float64), dtype=tf.float64)
...
class robot:
    ...
    def set_constants(self):
        ...
        self.body.vs = tf.zeros([1,3], dtype = tf.float64)
        self.body.wb = tf.zeros([1,3], dtype = tf.float64)
