...
W1=tf.Variable(tf.random_uniform([39,20], -0.001, 0.001,dtype=tf.float64), dtype=tf.float64)
W2=tf.Variable(tf.random_uniform([20,12], -0.001, 0.001,dtype=tf.float64), dtype=tf.float64)
...
class subleg(Rigidbody):
    def __init__(self,m=0.0,axis=tf.zeros((1,3),dtype=tf.float64),l=[tf.zeros((1,3),dtype=tf.float64),tf.zeros((1,3),dtype=tf.float64)],theta=tf.constant(0.0, tf.float64),omega=tf.constant(0.0, tf.float64),alpha=tf.constant(0.0, tf.float64)):
        Rigidbody.__init__(self,m=m)
        self.axis = axis
        self.l = l
        self.theta = theta
...
for time in range(timeN):
    ...
    inlay = tf.concat([inlay, tf.reshape(Destination, [3])], axis = 0)
    inlay = tf.concat([inlay, tf.reshape(R.body.Q, [9])], axis = 0)
    inlay = tf.concat([inlay, tf.reshape(R.body.rs, [3])], axis = 0)

    inlay = tf.reshape(inlay, [1,39])
...
for i in range(10000):
    _, printcost = sess.run([train_op, cost], feed_dict={Destination:[[1,0,0]], prs: nowrs, pvs:nowvs, pwb: nowwb, pQb: nowQb})
