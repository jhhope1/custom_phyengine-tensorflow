...
Ibinv = tf.matrix_inverse(tf.constant([[75.0e-5,0.,0.],
                               [0.,75.0e-5,0.],
                               [0.,0.,50.0e-5]],dtype=tf.float64))# 실제 값으로 바꿔야됨 ########################################################
...
class Rigidbody:
    def __init__(self,Q=tf.eye(3,dtype=tf.float64),wb=tf.zeros((1,3),dtype=tf.float64)):
        self.Q = Q
        self.wb = wb

class Robotbody(Rigidbody):
    def __init__(self, rs=tf.zeros((1,3), dtype=tf.float64), vs=tf.zeros((1,3), dtype=tf.float64), lbtomot = [tf.zeros((1,3),dtype=tf.float64) for _ in range(numLeg)]):
...
class subleg(Rigidbody):
    def __init__(self,axis=tf.zeros((1,3),dtype=tf.float64),l = [tf.zeros((1,3),dtype=tf.float64) for _ in range(2)],theta=0.0,omega=0.0,alpha=0.0):
        self.axis = axis
...
class robot:
    ...
    def set_constants(self):
        ...
        #Set lvectors
        self.body.lbtomot[0] = tf.constant([[0.065,0.065,0.02]],dtype=tf.float64)#############################3여기는 근사를 하면서 내려간 무게중심을 보정해주어야함
    ...
    def timeflow(self,t = 0.0):
        ...
        tot_lbtomots = []
        Feqc = tf.constant([0,0,-Mtot*9.81], dtype = tf.float64)
        ...
        for p in range(numLeg):
           ...
           for i in range(numsubleg):
               ...
               FrictionTemp = -tf.multiply(tf.scalar_mul( Fricscale , vs ), Collisionxy)
               Feqc += FrictionTemp
        ...
        '''
        return Flist, [x + self.body.rs for x in tot_lbtomots]
